...
## 4. よくあるカスタマイズ（Claude Codeに頼めばOK）

- **鑑定のルール・文体を変える** → `index.html` 内の `SYSTEM_PROMPT`（占い師AIへの指示文の本体）
- **入力項目を足す/減らす** → `index.html` のフォーム部分
- **納品前チェックの条件を変える** → `index.html` の `checkBtn` 周りのチェック処理
- **色・タイトルを変える** → `index.html` 冒頭の `<style>` 内 `:root`（`--gold` などの変数）

すべて `index.html` 1ファイルの中にあります。困ったら、このファイルを Claude Code に見せて相談してください。
